...
/**
 * 通过给定的moment 值来返回 年-月-日格式的日期
 * 如果传入则返回格式化日期，如果没有传入则返回当前时间的日期
 * @param {*moment对象} value
 *
 */
 ...
 * @param {*日期} date
 */
export const DateChangeToMoment = (date) => {
  return moment(date);
};

/**
 * 按照 对象内部的startAt 进行 排序
 * @param {*evnets 数据} listData
 */
export const sortDataListByStartAt = (listData) => {
  const events = listData.sort((item, nextitem) => {
    if (moment(item.startAt).isBefore(moment(nextitem.startAt))) {
      return -1;
    } else if (moment(item.startAt).isAfter(moment(nextitem.startAt))) {
      return 1;
    } else {
      return 0;
    }
...
/**
 * 按照 对象内部的startAt 和endAt 进行 排序 并拆分后形成对应的数组
 * @param {*evnets 数据} listData
 */
export const sortDataListByStartAtAndEndAt = (listData) => {
  const newDataList = [];

  listData.forEach((item) => {
    newDataList.push({ time: item.startAt, type: 'start', item });
    newDataList.push({ time: item.endAt, type: 'end', item });
  });
  const events = newDataList.sort((item, nextitem) => {
    if (moment(item.time).isBefore(moment(nextitem.time))) {
      return -1;
    } else if (moment(item.time).isAfter(moment(nextitem.time))) {
      return 1;
    } else {
      return 0;
    }
 ...
 * 定时监控计划过期情况
 */
export const reportActionisLated = (listData, rangeTime) => {
  const lated = [];
};
/**
 * 定时访问服务器进行数据备份的任务
...
/**
 * #获取全部
DeviceInfo.getDeviceInfo({domain: ''});
#获取单个
DeviceInfo.getDeviceInfo({domain: ''}).browserInfo;
 */
